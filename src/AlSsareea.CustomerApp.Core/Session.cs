...
public sealed class SessionManager(ISessionStorage storage, IAuthenticationApi auth) : ISessionManager
{
    ...
    public async Task ClearAsync(CancellationToken ct) { Volatile.Write(ref accessToken, null); await storage.ClearAsync(ct); }
}
